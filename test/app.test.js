...
test('generates the base app with the expected project structure', async () => {
  ...
  assert.equal(fs.existsSync(path.join(projectRoot, 'src/features/auth')), false);
...
test('prompts for the app name when one is not provided', async () => {
  ...
  yoAssert.fileContent(path.join(projectRoot, '.env.example'), 'VITE_APP_NAME=Prompt Driven App');
});
...
test('fails when the target directory already exists and is not empty', async () => {
  ...
  assert.equal(fs.existsSync(path.join(tmpDir, 'existing-app', 'keep.txt')), true);
});
